...
// album query
$result = query("SELECT id, parentid, folder, title FROM ". prefix('albums') ." ORDER BY sort_order");
  ...
  $id[$number] = $row['id']; 
	...
	if(getAlbumID() ===	$id[$number])
	  { $currentfolder = $folder[$number]; }
		...
		$result2 = query("SELECT COUNT(id) FROM ". prefix('images') ." WHERE albumid = $id[$number]");
     ...
     $sublevel_current = explode("/", $currentfolder);  
        ...
        AND $sublevel_current[0] === $folder[$top]))
      	...
      	// print link <strong> if selected
					...
					// link either for list or jump menu display
					if($option === "list")
					  { echo "<li".$active.">".$strong_start."<a href='".$albumlinkpath.$folder[$sub1]."'>".htmlspecialchars($title[$sub1])."</a>".$strong_end.$imagecount[$sub1]; }
      		else if ($option === "jump")
